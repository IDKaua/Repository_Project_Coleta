...
const Rastreio = () => {
  ...
  const tempoMin = duracaoRestante != null ? Math.max(0, Math.round(duracaoRestante / 60)) : null;

  const distLabel = distanciaRestante != null
  ...
  const handleConcluir = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/coletas/finalizar/${coletaAtiva.id}`, { method: "PUT" });
      if (response.ok) {
        toast.success("✨ Coleta finalizada com sucesso!");
        try { localStorage.setItem("rastreioOcultoUntil", String(Date.now() + 2 * 60 * 1000)); } catch (e) {}
  ...
  if (carregando) {
    return (
      <div className="rastreio-wrapper">
        <div className="rastreio-loading"><span className="rastreio-spinner" /><p>Carregando rastreio...</p></div>
      </div>
    );
  }
    ...
    return (
      <div className="rastreio-wrapper">
        <main className="rastreio-container" style={{ textAlign: "center", padding: "50px" }}>
          <h2>Redirecionando...</h2><p>Você não possui uma coleta ativa.</p>
        </main>
      </div>
    );
  }

  const isPendente = coletaAtiva.status === "PENDENTE" || coletaAtiva.status === "Pendente";

  // Determina as coordenadas do destino com base no que veio do Java
  const coordsDestino = coletaAtiva.latitude && coletaAtiva.longitude 
    ? [coletaAtiva.latitude, coletaAtiva.longitude] 
    : null;

  return (
    <div className="rastreio-wrapper">
      <main className="rastreio-container">
        <h1 className="rastreio-titulo">RASTREAMENTO DA COLETA #{coletaAtiva.id}</h1>
          ...
          <div className="rastreio-left">
            {isPendente ? (
              <>
                <div className="r-card r-card--amarelo">
                  ...
                  <p className="r-card-p">Sua solicitação foi recebida. A cooperativa está selecionando o coletor mais próximo.</p>
                </div>
                <div className="r-card">
                  <p className="r-card-subtitulo">ETAPAS DA SOLICITAÇÃO</p>
                  <div className="stepper-vertical">
                    <div className="step-item completed"><strong>Solicitação criada</strong></div>
                    <div className="step-item active"><strong>Triagem e escala</strong></div>
                    <div className="step-item pending"><strong>Coletor a caminho</strong></div>
                  </div>
                </div>
              </>
            ) : (
              <>
                <div className="r-card r-card--coletor">
                  ...
                  <div className="r-coletor-header">
                    <div className="r-avatar"><i className="fas fa-user" /></div>
                    <div className="r-coletor-dados">
                      <p className="r-coletor-nome">{coletaAtiva.coletor?.nome || "Carlos Silva"}</p>
                      <p className="r-coletor-veiculo">{coletaAtiva.coletor?.placaVeiculo ? `Van · ${coletaAtiva.coletor.placaVeiculo}` : "Van · ABC-1234"}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="r-card r-card--status">
                  <div className="r-status-header">
                    ...
                    <span>{chegou ? (
                    /* ── CHEGOU NO LOCAL ── */
                    coletaAtiva.status === "COLETADO" ? (
                      <>
                        <p className="r-chegou">Coletor confirmou a coleta! 🎉</p>
                        <p className="r-tempo-sub">Confirme o recebimento abaixo:</p>
                      ...
                      <>
                        <p className="r-chegou">Caminhão no local!</p>
                        <p className="r-tempo-sub">Aguardando coletor registrar a coleta...</p>
                      </>
                    )
                    ...
                    <>
                      ...
                      {distLabel && <p className="r-tempo-dist">{distLabel} restantes</p>}
                  ...
                  {chegou ? (
                    <>
                      ...
                      <button className="r-btn-confirmar" onClick={handleConcluir}>CONFIRMAR COLETA</button>
                    </>
                  ) : duracaoRestante === null ? (
                    <>
                      <p className="r-tempo">...</p>
            ...
            <div className="r-card">
              ...
              <div className="r-info-item">
                ...
                <div><p className="r-info-label">Tipo de Resíduo</p><p className="r-info-value">{coletaAtiva.tipoResiduo || "Eletrônicos"}</p></div>
              </div>
              <div className="r-info-item">
                <span className="r-info-icon">📍</span>
                <div><p className="r-info-label">Endereço</p><p className="r-info-value">{coletaAtiva.endereco || "Endereço cadastrado"}</p></div>
              </div>
              <div className="r-info-item">
                <span className="r-info-icon">📦</span>
                <div>
                  <p className="r-info-label">Status</p>
                  <p className={`r-info-value ${chegou ? "verde" : (isPendente ? "amarelo" : "azul")}`}>
                    {chegou ? "Coletado ✅" : (isPendente ? "Aguardando Coletor ⏳" : "Em rota 🚛")}
                  </p>
                </div>
              </div>
            </div>
